-- Fix RETURN TABLE types to match actual table column types
...
BEGIN
    ...
    -- Prevent duplicate - cast both sides to TEXT
    IF EXISTS(
        SELECT 1 FROM user_tickets ut
        WHERE ut.user_id::text = p_buyer_id::text
    ...
    SELECT
        gen_random_uuid(),
        p_buyer_id::text,               -- Cast to TEXT for insertion
        ut.event_id,
        ut.event_name,
        ut.event_date,
        ut.event_location,
...
SELECT 'Function fixed with TEXT user_id!' as status;
