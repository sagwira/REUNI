-- Fix type casting - id column is UUID, not TEXT
...
BEGIN
    ...
    IF EXISTS(
        ...
        WHERE ut.user_id = p_buyer_id
          AND ut.transaction_id = p_transaction_id
          AND ut.purchased_from_seller_id = p_seller_id
    ) THEN
        RAISE EXCEPTION 'Buyer ticket already exists for transaction %', p_transaction_id;
    END IF;

    -- Create buyer's ticket (keep UUIDs as UUIDs, not text)
    RETURN QUERY
    ...
    SELECT
        gen_random_uuid(),              -- UUID type
        p_buyer_id,                     -- UUID type
        ut.event_id,
        ut.event_name,
        ut.event_date,
        ut.event_location,
        ...
        'available',
        NULL,
        p_seller_id,                    -- UUID type
        p_transaction_id,               -- UUID type
...
SELECT 'Function fixed with correct UUID types!' as status;
