-- Fix buyer ticket creation function - Add event_id column
-- This was causing: ERROR: null value in column "event_id" violates not-null constraint
...
BEGIN
    ...
    IF EXISTS(
        ...
        WHERE ut2.user_id = p_buyer_id::text
    ...
    -- Create the buyer's ticket (INCLUDING event_id!)
    RETURN QUERY
    ...
    SELECT
        gen_random_uuid(),
        p_buyer_id::text,
        ut.event_id,              -- ← CRITICAL: Include event_id
        ut.event_name,
        ut.event_date,
        ut.event_location,
        ut.ticket_type,
