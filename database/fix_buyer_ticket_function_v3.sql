-- Fix type casting - table columns are TEXT, function params are UUID
...
BEGIN
    ...
    -- Prevent duplicate ticket creation (cast TEXT columns to UUID for comparison)
    IF EXISTS(
    ...
    -- Create buyer's ticket (cast UUIDs to TEXT for insertion)
    RETURN QUERY
    ...
    SELECT
        ...
        'available',
        NULL,
        p_seller_id::text,              -- Cast to TEXT
        p_transaction_id::text,         -- Cast to TEXT
...
SELECT 'Function fixed with proper type casting!' as status;
