-- Working version - table columns are TEXT, params are UUID
-- Cast params to TEXT for comparisons and insertions
...
BEGIN
    ...
    -- Prevent duplicate ticket creation (cast params to TEXT for comparison)
    IF EXISTS(
        SELECT 1 FROM user_tickets ut
        WHERE ut.user_id = p_buyer_id::text
          AND ut.transaction_id = p_transaction_id::text
          AND ut.purchased_from_seller_id = p_seller_id::text
    ) THEN
        RAISE EXCEPTION 'Buyer ticket already exists for transaction %', p_transaction_id;
    END IF;

    -- Create buyer's ticket
    RETURN QUERY
    ...
    SELECT
        gen_random_uuid(),              -- id is UUID
        p_buyer_id,                     -- user_id is UUID  
        ut.event_id,                    -- event_id is UUID
        ut.event_name,
        ut.event_date,
        ut.event_location,
        ut.organizer_name,
        ...
        'available',
        NULL,
        p_seller_id,                    -- purchased_from_seller_id is UUID
        p_transaction_id,               -- transaction_id is UUID
...
SELECT 'Function working!' as status;
