...
//  REUNI
//
//  Password reset OTP verification view
//
...
struct PasswordResetOTPView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .sheet(isPresented: $showResetPassword) {
                ResetPasswordView(email: email, authManager: authManager)
            }
    ...
    func verifyCode() async {
        ...
        do {
            let success = try await authManager.verifyPasswordResetOTP(email: email, token: code)

            if success {
                print("✅ Password reset OTP verified - showing password reset view")
                isVerifying = false
    ...
    func resendCode() async {
        do {
            try await authManager.resendPasswordResetOTP(email: email)
            canResend = false
            resendCountdown = 60
            startResendTimer()
            clearOTPFields()
...
#Preview {
    PasswordResetOTPView(email: "test@example.com", authManager: AuthenticationManager())
}
